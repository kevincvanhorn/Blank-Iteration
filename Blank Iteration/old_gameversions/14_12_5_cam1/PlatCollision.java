...
public class PlatCollision{ //for a Rectangle
	...
	private int x, y, width, height, xChar, yChar, xList, yList;
	...
	public PlatCollision(int w, int h, int ix, int iy, ArrayList<Rectangle> platListIn){//int ix, int iy, int w, int h, Rectangle plat){
		width = w;//image.getWidth();
		height = h;//image.getHeight();
		allBounds = new Rectangle(ix, iy, width, height);
	...
	public void calcBounds(int x, int y){
		botBounds = new Rectangle(x+1, y+height-1, width-2, 1);//
		topBounds = new Rectangle(x+1, y, width-2, 1);
		leftBounds = new Rectangle(x, y, 1, height-2);
		rightBounds = new Rectangle(x+width-1, y+1, 1, height-2);//
		stepsOff(x);
	}
	...
	public void update(int modX, int modY){
		for(int i = 0; i<platList.size(); i++){
			xList = (int)platList.get(i).getX();
			yList = (int)platList.get(i).getY();
			//System.out.println("plat update"+yList+" "+modY);
			platList.get(i).setLocation(xList+modX, yList+modY);
		}
	}
	...
	public String toStringCol(int i){
		for(int c=0;c<=i;c++){
			string += platList.get(i).toString();
		}
		return string;
	}
}
